...
//
double dotProductSeq(const std::vector<double> & vectorA, const std::vector<double> & vectorB)
{
    double res = 0;
...
//
double dotProductParallel(const std::vector<double> & vectorA, const std::vector<double> & vectorB)
{
	double res = 0;
...
#pragma omp parallel num_threads(NUM_THREADS) 
	{
		double localRes = 0;
		int nbStepsPerThread = SIZE / omp_get_num_threads();
		int firstId = nbStepsPerThread * omp_get_thread_num();
		for (int i = firstId; i < firstId + nbStepsPerThread; i++)
		{
			localRes += vectorA[i] * vectorB[i];
		}
#pragma omp atomic
		res += localRes;
	}
...
//
double dotProductParallelFor(const std::vector<double> & vectorA, const std::vector<double> & vectorB)
{
	double res = 0;
...
//
double dotProductawful(const std::vector<double> & vectorA, const std::vector<double> & vectorB)
{
	double res = 0;
...
int main()
{
    ...
    std::vector<double>    vectorA(SIZE);
	std::vector<double>    vectorB(SIZE);
