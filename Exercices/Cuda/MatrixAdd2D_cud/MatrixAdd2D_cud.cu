...
{
	// TODO implémentez la version 2D du kernel
	// calculer x_global, y_global
	// puis indice_global
}
...
int main(void)
{
	...
	std::cout << "nb threads: " << threadsPerBlock.x << '\t' << threadsPerBlock.y << '\t' << threadsPerBlock.z << std::endl;
	std::cout << "nb blocs  : " << blocksPerGrid.x << '\t' << blocksPerGrid.y << '\t' << blocksPerGrid.z << std::endl;
